...
def get_features(model, x):
    def register_hook(module, name: str):
        def hook(module, input, output):
            ...
            summary[name]["output"] = output
            summary[name]["parameter"] = module
    ...
    # register hook
    named_apply(model, register_hook, mod_name=None)
...
def named_summary(model, input_size, batch_size=-1, device="cuda"):
    def register_hook(module, name: str):
        def hook(module, input, output):
            module_idx = len(summary)
            class_name = str(module.__class__).split(".")[-1].split("'")[0]
            m_key = "%s-%s-%i" % (name, class_name, module_idx + 1)
